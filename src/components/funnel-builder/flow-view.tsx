...
const inputClass = 'h-8 min-w-0 rounded-lg border border-border bg-muted/40 px-2 text-[10px] text-foreground outline-none transition focus:border-violet-400/50'
      ...
      {group.conditions.map((condition, index) => {
        ...
        return (
          <div key={condition.id} className="grid grid-cols-[82px_minmax(86px,1fr)_106px_minmax(76px,1fr)_28px] gap-1">
            ...
            <button type="button" aria-label="Excluir condição" onClick={() => onChange({ ...group, conditions: group.conditions.filter((_, current) => current !== index) })} className="flex size-7 items-center justify-center rounded-lg text-muted-foreground/60 hover:bg-red-500/10 hover:text-red-500"><Trash2 className="size-3" /></button>
            {!!keys.length && <datalist id={`flow-keys-${condition.source}`}>{keys.map((key) => <option key={key} value={key} />)}</datalist>}
          </div>
        )
      })}
      <button type="button" onClick={() => onChange({ ...group, conditions: [...group.conditions, defaultCondition(answerKeys[0])] })} className="inline-flex h-7 items-center gap-1 rounded-lg border border-dashed border-border px-2 text-[9px] font-semibold text-muted-foreground hover:border-violet-300/40 hover:text-violet-600"><Plus className="size-3" /> Condição</button>
    </div>
  )
}
...
export function FlowView({ document, activePageId, onSelectPage, onChange }: FlowViewProps) {
  ...
  if (!selectedPage) {
    return <div className="flex min-h-full items-center justify-center text-sm text-muted-foreground/60">Crie uma página para configurar o fluxo.</div>
  }
  ...
  return (
    <div className="mx-auto w-full max-w-[1460px] space-y-5 p-8" onClick={(event) => event.stopPropagation()}>
      <section className="flex items-center justify-between rounded-2xl border border-border bg-card/95 p-4 shadow-lg shadow-black/10">
        <div>
          <div className="flex items-center gap-2 text-sm font-bold text-foreground"><GitBranch className="size-4 text-violet-500" /> Fluxo e ramificações</div>
          <p className="mt-1 text-[10px] text-muted-foreground">Condições são avaliadas por prioridade; faixa de resultado e destino padrão vêm em seguida.</p>
        </div>
        <div className={`flex items-center gap-2 rounded-xl border px-3 py-2 text-[10px] font-semibold ${validation.valid ? 'border-emerald-400/15 bg-emerald-400/[.06] text-emerald-600' : 'border-amber-400/20 bg-amber-400/[.06] text-amber-700'}`}>
        ...
        <div className="space-y-5">
          <section className="rounded-2xl border border-border bg-card/95 p-5">
            <div className="mb-4 flex items-center justify-between">
              <div><h2 className="text-xs font-bold text-foreground">Mapa de páginas</h2><p className="mt-1 text-[9px] text-muted-foreground/60">Selecione um nó para editar as suas saídas.</p></div>
              <label className="flex items-center gap-2 text-[9px] text-muted-foreground">
                Entrada
            ...
            <div className="grid grid-cols-3 gap-3">
              {pages.map((page, index) => {
                ...
                return (
                  <button type="button" key={page.id} onClick={() => onSelectPage(page.id)} className={`group min-h-32 rounded-2xl border p-4 text-left transition hover:-translate-y-0.5 ${page.id === selectedPage.id ? 'border-violet-400/45 bg-violet-500/[.09] shadow-lg shadow-violet-950/10' : pageIssues.length ? 'border-amber-400/20 bg-amber-400/[.035]' : 'border-border bg-muted/20 hover:border-border'}`}>
                    <div className="flex items-center justify-between">
                      <span className="text-[8px] font-bold uppercase tracking-[.16em] text-muted-foreground/60">Página {index + 1}</span>
                      {isEntry ? <span className="rounded-full bg-emerald-400/10 px-2 py-0.5 text-[7px] font-bold text-emerald-600">ENTRADA</span> : pageIssues.length > 0 ? <AlertTriangle className="size-3 text-amber-500" /> : null}
                    </div>
                    <p className="mt-3 truncate text-xs font-bold text-foreground">{page.name}</p>
                    <div className="mt-3 space-y-1">
                      {pageOutgoing.slice(0, 2).map((connection) => <span key={connection.id} className="flex items-center gap-1 truncate text-[8px] text-muted-foreground"><ArrowRight className="size-2.5 shrink-0 text-violet-400/70" />{connection.isDefault ? 'Padrão: ' : 'Se: '}{pageLabel(document, connection.targetPageId)}</span>)}
                      {!pageOutgoing.length && <span className="text-[8px] text-muted-foreground/50">Página final</span>}
                    </div>
                  </button>
                )
              })}
          ...
          <section className="rounded-2xl border border-border bg-card/95 p-5">
            <div className="flex items-center justify-between">
              <div><h2 className="flex items-center gap-2 text-xs font-bold text-foreground"><Trophy className="size-3.5 text-amber-500" /> Pontuação condicional</h2><p className="mt-1 text-[9px] text-muted-foreground/60">Regras por opção continuam no elemento; aqui você combina respostas, variáveis e UTMs.</p></div>
              <button type="button" onClick={addScoreRule} className="inline-flex h-8 items-center gap-1.5 rounded-lg border border-border px-3 text-[9px] font-semibold text-muted-foreground hover:border-violet-300/40 hover:text-violet-600"><Plus className="size-3" /> Regra</button>
            </div>
            <div className="mt-4 space-y-3">
              {(flowDefinition?.scoringRules ?? []).map((rule) => (
                <div key={rule.id} className="rounded-xl border border-border bg-muted/30 p-3">
                  <div className="mb-3 flex items-center gap-2">
                    ...
                    <label className="flex items-center gap-1 text-[9px] text-muted-foreground">Pontos <input type="number" value={rule.points} onChange={(event) => updateScoreRule(rule.id, (current) => ({ ...current, points: Number(event.target.value) }))} className={`${inputClass} w-20`} /></label>
                    <button type="button" aria-label="Excluir regra de pontuação" onClick={() => updateFlow((flow) => ({ ...flow, scoringRules: (flow.scoringRules ?? []).filter((item) => item.id !== rule.id) }))} className="flex size-8 items-center justify-center rounded-lg text-muted-foreground/60 hover:bg-red-500/10 hover:text-red-500"><Trash2 className="size-3.5" /></button>
                  </div>
              ...
              {!flowDefinition?.scoringRules?.length && <div className="rounded-xl border border-dashed border-border p-5 text-center text-[9px] text-muted-foreground/50">Nenhuma regra global de score.</div>}
            </div>
          </section>

          <section className="rounded-2xl border border-border bg-card/95 p-5">
            <div className="flex items-center justify-between">
              <div><h2 className="flex items-center gap-2 text-xs font-bold text-foreground"><Play className="size-3.5 text-cyan-500" /> Simulador de percurso</h2><p className="mt-1 text-[9px] text-muted-foreground/60">Informe respostas/variáveis em JSON. Para um botão “Ir para página”, use <code>{'"__actions":{"slug-origem":"slug-destino"}'}</code>.</p></div>
              <button type="button" onClick={runSimulation} className="inline-flex h-8 items-center gap-1.5 rounded-lg bg-cyan-500/10 px-3 text-[9px] font-bold text-cyan-600 hover:bg-cyan-500/15"><Play className="size-3" /> Simular</button>
            </div>
            <textarea value={simulatorInput} onChange={(event) => setSimulatorInput(event.target.value)} spellCheck={false} className="mt-4 min-h-24 w-full rounded-xl border border-border bg-muted/30 p-3 font-mono text-[10px] leading-5 text-foreground outline-none focus:border-cyan-400/30" />
            {simulatorError && <p className="mt-2 text-[9px] text-rose-700">{simulatorError}</p>}
            {simulation && <div className={`mt-3 rounded-xl border p-3 ${simulation.stopReason === 'terminal' ? 'border-emerald-400/15 bg-emerald-400/[.04]' : 'border-amber-400/20 bg-amber-400/[.04]'}`}><div className="flex items-center justify-between text-[9px]"><span className="font-semibold text-foreground">{pathLabel(document, simulation)}</span><span className="rounded-full bg-muted px-2 py-1 font-mono text-muted-foreground">score {simulation.score}</span></div><p className="mt-2 text-[8px] text-muted-foreground/60">Parada: {simulation.stopReason}{simulation.resultKey ? ` · resultado: ${simulation.resultKey}` : ''}</p></div>}
          </section>
        </div>

        <aside className="space-y-4">
          <section className="rounded-2xl border border-border bg-card p-4 shadow-lg shadow-black/10">
            <div className="flex items-start justify-between">
              <div><span className="text-[8px] font-bold uppercase tracking-[.14em] text-violet-500">Saídas da página</span><h2 className="mt-1 text-sm font-bold text-foreground">{selectedPage.name}</h2></div>
              <button type="button" onClick={addConnection} disabled={pages.length < 2} className="inline-flex size-8 items-center justify-center rounded-lg border border-border text-muted-foreground hover:border-violet-400/30 hover:text-violet-600 disabled:opacity-30" aria-label="Adicionar rota"><Plus className="size-4" /></button>
            </div>
            <div className="mt-4 space-y-3">
              {outgoing.map((connection, index) => (
                <div key={connection.id} className="rounded-xl border border-border bg-muted/30 p-3">
                  <div className="flex items-center gap-2">
                    <span className={`flex size-6 items-center justify-center rounded-lg ${connection.isDefault ? 'bg-cyan-400/10 text-cyan-600' : 'bg-violet-400/10 text-violet-600'}`}>{connection.isDefault ? <Route className="size-3" /> : <GitBranch className="size-3" />}</span>
                    ...
                    <button type="button" aria-label="Excluir rota" onClick={() => removeConnection(connection.id)} className="flex size-8 items-center justify-center rounded-lg text-muted-foreground/60 hover:bg-red-500/10 hover:text-red-500"><Trash2 className="size-3.5" /></button>
                  </div>
                  <div className="mt-2 flex items-center justify-between">
                    ...
                    <label className="flex items-center gap-1 text-[8px] text-muted-foreground/60">Prioridade <input type="number" value={connection.priority ?? index} onChange={(event) => updateConnection(connection.id, (current) => ({ ...current, priority: Number(event.target.value) }))} className={`${inputClass} w-14`} /></label>
                  </div>
                  {!connection.isDefault && <div className="mt-3 border-t border-border pt-3"><ConditionEditor group={connection.condition ?? { operator: 'and', conditions: [] }} answerKeys={answerKeys} variableKeys={variableKeys} onChange={(condition) => updateConnection(connection.id, (current) => ({ ...current, condition }))} /></div>}
                </div>
              ))}
              {!outgoing.length && <div className="rounded-xl border border-dashed border-border p-5 text-center"><CircleDot className="mx-auto size-5 text-muted-foreground/50" /><p className="mt-2 text-[9px] text-muted-foreground/60">Esta é uma página final. Adicione uma rota para continuar.</p></div>}
            </div>
          </section>

          <section className="rounded-2xl border border-border bg-card p-4">
            <div className="flex items-center justify-between">
              <div><span className="text-[8px] font-bold uppercase tracking-[.14em] text-amber-600">Resultados por score</span><p className="mt-1 text-[9px] text-muted-foreground">Avaliados antes do destino padrão.</p></div>
              <button type="button" onClick={addResultRange} disabled={pages.length < 2} className="flex size-8 items-center justify-center rounded-lg border border-border text-muted-foreground hover:text-amber-600 disabled:opacity-30"><Plus className="size-3.5" /></button>
            </div>
            <div className="mt-3 space-y-2">
              {selectedRanges.map((range) => (
                <div key={range.id} className="rounded-xl border border-border bg-muted/30 p-3">
                  <div className="flex items-center gap-1.5">
                    ...
                    <button type="button" aria-label="Excluir resultado" onClick={() => updateFlow((flow) => ({ ...flow, resultRanges: (flow.resultRanges ?? []).filter((item) => item.id !== range.id) }))} className="flex size-8 items-center justify-center rounded-lg text-muted-foreground/60 hover:bg-red-500/10 hover:text-red-500"><Trash2 className="size-3.5" /></button>
                  </div>
              ...
              {!selectedRanges.length && <p className="rounded-xl border border-dashed border-border p-4 text-center text-[9px] text-muted-foreground/50">Nenhuma faixa nesta página.</p>}
            </div>
          </section>

          {!validation.valid && (
            <section className="rounded-2xl border border-amber-400/15 bg-amber-400/[.035] p-4">
              <h3 className="flex items-center gap-2 text-[10px] font-bold text-amber-700"><AlertTriangle className="size-3.5" /> Corrija antes de publicar</h3>
              <ul className="mt-3 space-y-2">
                {selectedIssues.slice(0, 8).map((issue, index) => <li key={`${issue.code}-${issue.connectionId ?? issue.pageId ?? index}`} className="text-[9px] leading-4 text-amber-800/70">• {issue.message}</li>)}
              </ul>
            </section>
          )}
        </aside>
