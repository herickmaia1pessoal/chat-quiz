  ...
  return (
    <div className="w-full rounded-3xl border border-border bg-card p-6 sm:p-8 backdrop-blur-2xl shadow-2xl space-y-5 animate-in fade-in slide-in-from-bottom-4 duration-300">
      ...
      <div className="rounded-2xl border border-border bg-muted/30 p-5 space-y-3">
        ...
        {text && <p className="text-foreground text-sm sm:text-base italic leading-relaxed">{text}</p>}

        {(author || authorRole) && (
          <div className="flex items-center gap-2.5 pt-1">
            {avatarUrl ? (
              // eslint-disable-next-line @next/next/no-img-element
              <img src={avatarUrl} alt={author || 'Avatar'} className="h-9 w-9 rounded-full object-cover border border-border" />
            ) : (
            ...
            <div>
              {author && <p className="text-foreground text-xs font-semibold">{author}</p>}
              {authorRole && <p className="text-muted-foreground text-[11px]">{authorRole}</p>}
            </div>
          </div>
        )}
      </div>
